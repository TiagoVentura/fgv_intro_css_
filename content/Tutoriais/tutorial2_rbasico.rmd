...
A maior parte dos usuários iniciantes em R costumam digitar seus códigos diretamente na linha de comando. **Esta é uma péssimoa opção**. 
...
Se o seu prompt começar a parecer um símbolo `+` por engano, basta clicar no console e pressionar a tecla `esc` no teclado quantas vezes forem necessárias para retornar ao prompt. 
...
Um pacote em R é simplemente um conjunto de funções - e as vezes dados - disponibilizados por outros usuários ou pela própria equipe de manutenção e criação do R. Estas funções em gerais são colocadas juntas em um pacote porque servem a um propósito específico: um modelo estatístico novo, um conjunto de funções para visualização de dados, um pacote com funções para acessar dados eleitorais. 

Instalar um pacote consiste, basicamente, em fazerm download do pacote -- um conjunto de funções -- para seu computador. Por isso, você instala o pacote somente uma vez. No entanto, cada vez que você abre seu R, você precisa lembrá-lo de que você possui determinados pacotes instalados, e, ao invés de baixar tudo outra vez, você pode somente ativá-los. Esta é a diferença entre `install.packackages()` e `library()


## Instalar Pacotes Via Cran.

O CRAN é o repositório oficial do R. Lá estão todos os pacotes que cumprem determinados requisitos da equipe do R
...
Todas as funções e pacotes de R possuem uma documentação. Esta documentação explica como utilizar determinada função, quais seus argumentos, e provê exemplos de uso. Esta ajuda aparecerá em painel inferior direito. Você pede desta forma
...
Cada linguagem de programação possui regras próprias sobre como criar, acessar e guardar variáveis no seu ambiente de trabalho. Em R, variáveis, ou objetos, são criados atribuindo determinado **dado** a um determinado **nome**, e você pode acessá-los exatamente por nome atribuído. 
...
Acima, nós atribuímos os valores 5, 7, e "Tiago Ventura" aos objetos de **nome** x, y, e nome.  Esta é a lógica: object_name <- value`

É possível criar variáveis utilizando `=`. Porém, o uso deste operador não é recomendado, visto que `=` também possui diversas outras funcionalidade em R. 
...
Crie nomes de forma efetiva, que descreveram os objetos do seu ambiente de trabalho. Nosso objetivo enquanto acadêmicos é produzir ciência e pesquisa, e parte do processo de criar conhecimento consiste em compartilhar suas pesquisas incluindo seus códigos. Portanto, crie nomes em que qualquer pessoa entenderá a que este objeto se refere. 
...
Casa objeto em R possui uma **Classe**. Em síntese, a classe descreve qual tipo de valores esse objeto está armazenando. De forma práticas, veremos objetos em R  nas seguintes classes:
...
Classas são importante porque o R está programado para lidar com estes tipos de variáveis. Em resumo, o R entende que determinado tipo de objeto é numérico, e portanto, este objetivo pode realizar determinadas funções. Quando criamos uma função em R precisamos específicar qual tipo de objeto esta funcão aceita como input, e esta programação é que faz das classes um peça fundamental em qualquer linguagem de programação. 
...
Expressões booleanas são valores lógicos que retornam de determinada relação é verdadeira ou falsa. Estão expressão booleanas são a base de diversas linguagens de programação, incluindo **R**. Usaremos expressões boleanas para filtrar vetores ou bancos de dados, escrever loops, funções, 
relações condicionais, ou simplesmente comparar dois números. 
...
Por outro lado, operadores lógicos nos permite combinar multiplas análises usando operadores booleanos. Operadores lógicos são:
...
Meio confuso né? Sim. Concordo. Por isso, vamos usa um pacote chamado [dplyr](https://dplyr.tidyverse.org/) para manipulação de bancos de dados. Vamos chegar lá nas próximas aulas. 
...
Para concluir, estes são somente alguns poucos exemplos. Há multiplas funcões, algumas para abrir o mesmo tipo de arquivo, disponíveis para baixar. Por exemplo, vamos usar mais a função `read_csv`, ao invés do `read.csv`,  do pacote `readr`, porque está funciona de forma mais rápida quando abrindo um conjunto grande de dados. 
